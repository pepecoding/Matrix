...
public class LogFileCreator {
	...
	public ArrayList<LogForExample> getLogList(int size){
		...
		for(int i=1; i<=size; i++){
			...
			long connectionTime = System.currentTimeMillis()-(60*60*i);
			String connectorIp = getRandomNumber(1, 255)+"."+getRandomNumber(1, 255)+"."+getRandomNumber(1, 255)+"."+getRandomNumber(1, 255);
	...
	public void createFile(ArrayList<LogForExample> logList, String filePath){
		...
		try {
			...
			for (LogForExample log : logList) {
				stringToWrite += log.getConnectionId()+" "+log.getConnectionTime()+
							 " "+log.getConnectorIp()+" "+log.getSpecId()+"\n";
			}
